 ...
 *
 * A `ContinuousRange` can be _open_, _half-open_, or _closed_ depending on type of its limits.
 *
 ...
 */
internal open class GenericRange<T : Comparable<T>, R : GenericRange<T, R>>(
    val start: Limit<T>,
    val end: Limit<T>,
    val ctor: (Limit<T>, Limit<T>) -> R,
) {
    ...
    fun intersect(that: R): R? {
        ...
        return if (isEmpty(newStart, newEnd)) null else ctor(newStart, newEnd)
    }
